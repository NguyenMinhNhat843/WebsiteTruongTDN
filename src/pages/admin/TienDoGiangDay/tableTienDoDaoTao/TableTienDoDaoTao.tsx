...
const TableTienDoDaoTao = () => {
  ...
  // Api xuất file excel
  const { mutate: exportExcelData, isPending: isLoadingExportExcel } = $api.useMutation(
    'get',
  ...
  const tableData: DataRow[] = useMemo(() => {
    ...
    return trainingPlan.map((row) => {
      const tongSoTiet = (row.subject.testHours || 0) + row.subject.theoryHours + row.subject.practiceHours
  ...
  // Hàm xóa buổi học
  const handleRemoveClassSubjectSession = (itemIndex: number, sessionIndex: number) => {
    setTable((prevTable) =>
  ...
  const handleToggleWeek = (itemIndex: number, sessionIndex: number, weekNumber: number) => {
  ...
  const handleChangeRoom = (itemIndex: number, sessionIndex: number, roomIdValue: string) => {
  ...
  const { mutate: upsertPlanTraining, isPending: isPendingUpsertPlanTraining } = $api.useMutation(
    'post',
      ...
      const sessions = item.sessions
        ...
        .map((session) => {
          ...
          return {
            ...
            dayOfWeek: session.thu as UpsertPlanTrainingDto['items'][number]['sessions'][number]['dayOfWeek'],
            endPeriod,
            shift,
            startPeriod,
            schedules,
      ...
      <div className="w-full rounded-lg bg-white p-4 shadow-sm">
        {isLoadingTrainingPlan ? (
          <div className="flex w-full items-center justify-center py-8">
            <div className="flex items-center gap-3 rounded-full border border-slate-100 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-500 shadow-sm">
        ...
        <div className="mt-4 flex justify-end">
          ...
          <button
            ...
            className="ms-2 flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-indigo-400"
          >
            {isPendingUpsertPlanTraining && (
              <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />
            )}
            {isPendingUpsertPlanTraining ? 'Đang lưu...' : 'Lưu tiến độ đào tạo'}
          </button>
        </div>
      </div>
    </PageShell>
