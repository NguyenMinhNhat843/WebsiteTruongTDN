...
export default function TuitionDashboard() {
  ...
  const { data: tuitionPeriods, isLoading: isLoadingPeriods } = $api.useQuery('get', '/tuition-periods')
  ...
  const isGlobalLoading = isLoadingPeriods || isOverviewLoading || isTrendLoading
  const kpis = overview?.kpis

  return (
    <PageShell
      title="Tổng quan học phí"
      sub="Theo dõi công nợ, doanh thu thực tế và quản lý tiến độ đóng học phí của học sinh."
      icon={CalendarPlus}
      renderRight={
        <div className="relative min-w-[200px]">
          <select
            className="w-full appearance-none rounded-xl border border-slate-200 bg-white px-4 py-2.5 pr-10 text-sm font-semibold text-slate-700 shadow-sm transition-all outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
    ...
    >
      <div className="min-h-screen">
            ...
            <>
              ...
              <div className="group rounded-xl border border-slate-200/60 bg-white p-5 shadow-sm transition-all duration-200 hover:border-slate-300 hover:shadow-md">
                ...
                <p className="mt-2 text-2xl font-bold tracking-tight text-slate-900 tabular-nums">
                  ...
                  <span className="ml-1 text-sm font-normal text-slate-400">VND</span>
                </p>
              ...
              <div className="group rounded-xl border border-slate-200/60 bg-white p-5 shadow-sm transition-all duration-200 hover:border-emerald-200 hover:shadow-md">
                ...
                <p className="mt-2 text-2xl font-bold tracking-tight text-emerald-600 tabular-nums">
                  ...
                  <span className="ml-1 text-sm font-normal text-emerald-500">VND</span>
                </p>
                <div className="mt-2">
              ...
              <div className="group rounded-xl border border-slate-200/60 bg-white p-5 shadow-sm transition-all duration-200 hover:border-rose-200 hover:shadow-md">
                ...
                <p className="mt-2 text-2xl font-bold tracking-tight text-rose-600 tabular-nums">
                  ...
                  <span className="ml-1 text-sm font-normal text-rose-500">VND</span>
                </p>
              ...
              <div className="group rounded-xl border border-slate-200/60 bg-white p-5 shadow-sm transition-all duration-200 hover:border-blue-200 hover:shadow-md">
                ...
                <div className="mt-3 flex items-center justify-between text-xs">
                  <span className="font-medium text-slate-400">Tiến độ đợt thu</span>
                  <span className="font-bold text-blue-600 tabular-nums">{kpis?.collectionRate ?? 0}%</span>
                </div>
              </div>
            </>
          )}
              ...
              {overview?.statusDistribution?.map((item) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    {item.status === 'paid' && (
                      <span className="inline-flex items-center rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-600/10 ring-inset">
                        Đã hoàn thành
                      </span>
                    )}
                    {item.status === 'partial' && (
                      <span className="inline-flex items-center rounded-md bg-amber-50 px-2 py-1 text-xs font-semibold text-amber-700 ring-1 ring-amber-600/10 ring-inset">
                        Mới đóng 1 phần
                      </span>
                    )}
                    {item.status !== 'paid' && item.status !== 'partial' && (
                      <span className="inline-flex items-center rounded-md bg-rose-50 px-2 py-1 text-xs font-semibold text-rose-700 ring-1 ring-rose-600/10 ring-inset">
                        Chưa thanh toán
                      </span>
                    )}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200/60 bg-white p-5 shadow-sm">
            <h3 className="mb-4 text-sm font-bold tracking-wider text-slate-400 uppercase">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-xs font-bold tracking-wider text-slate-400 uppercase">
                    <th className="pb-3 font-semibold">Mã học sinh</th>
                    <th className="pb-3 font-semibold">Kênh thanh toán</th>
                    <th className="pb-3 text-right font-semibold">Số tiền nhận</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {overview?.recentPayments?.map((p) => (
                    <tr key={p.id} className="group transition-colors hover:bg-slate-50/60">
                      <td className="py-3.5 font-semibold text-slate-900 tabular-nums">HS-{p.student?.id}</td>
                      <td className="py-3.5">
                        <span className="rounded-md border border-slate-100 bg-slate-50 px-2 py-0.5 text-xs font-medium text-slate-600">
                          {p.method}
                        </span>
                      </td>
                      <td className="py-3.5 text-right font-bold tracking-tight text-emerald-600 tabular-nums">
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-xs font-bold tracking-wider text-slate-400 uppercase">
                    ...
                    <th className="pb-3 text-right font-semibold">Dư nợ còn lại</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {overview?.topDebtors?.map((d) => (
                    <tr key={d.id} className="group transition-colors hover:bg-slate-50/60">
                      <td className="py-3.5 font-semibold text-slate-900 tabular-nums">HS-{d.student?.id}</td>
                      <td className="py-3.5">
                        <span
                        ...
                        >
                          {d.status === 'partial' ? 'Nợ một phần' : 'Chưa đóng nợ'}
                        </span>
                      </td>
                      <td className="py-3.5 text-right font-bold tracking-tight text-rose-600 tabular-nums">
