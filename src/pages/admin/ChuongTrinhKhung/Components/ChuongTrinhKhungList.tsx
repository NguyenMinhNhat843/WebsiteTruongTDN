...
const ChuongTrinhKhungList = ({ data }: Props) => {
  ...
  return (
    <div className="w-96 shrink-0 space-y-3 p-1"> {/* Tăng nhẹ width và khoảng cách giữa các card */}
      {/* Trạng thái trống (Empty State) với UI chỉn chu hơn */}
      {data.length === 0 && (
        <div className="bg-slate-50 rounded-2xl border border-dashed border-slate-200 p-12 text-center flex flex-col items-center justify-center">
          ...
          <p className="text-slate-500 font-medium text-sm">Không có kết quả phù hợp</p>
          <p className="text-slate-400 text-xs mt-1">Vui lòng thử lại với bộ lọc khác.</p>
        </div>
      )}
        ...
        return (
          <button
            ...
            className={`w-full text-left rounded-xl border p-4 transition-all duration-200 ease-in-out relative overflow-hidden group
              ${isSel 
                ? "border-blue-500 bg-gradient-to-r from-blue-50/30 to-white shadow-md ring-1 ring-blue-500/30" 
                : "border-slate-200 bg-white hover:border-slate-300 hover:shadow-md hover:-translate-y-[1px]"
              }`}
          >
            {/* Thanh màu highlight nhỏ ở cạnh trái khi được chọn */}
            {isSel && <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-500" />}
            ...
            <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-100 mt-2">
              <div className="flex items-center gap-1.5 text-slate-500 max-w-[70%] truncate">
                ...
                <span className="truncate">Ngành: <span className="font-medium text-slate-700">{fw.major?.majorName || "Chưa cập nhật"}</span></span>
              </div>
