...
const TableDanhSachHocSinh = () => {
  ...
  const { mutate: removeStudentFromClass, isPending: isRemovingStudent } = $api.useMutation(
    'patch',
    '/students/{id}',
  )
  ...
  const handleKickStudent = (studentId: number) => {
    ...
    const isConfirmed = window.confirm('Bạn có chắc chắn muốn xóa học sinh này khỏi lớp?')
  ...
  const dataDanhSachHocSinh = useMemo(() => {
    const rawStudents = studentsInLopHoc?.data || []
    ...
    exportStudentGrade(
      ...
      },
      {
        ...
        onError: (error) => {
          ...
          alert('Không thể xuất tệp Excel bảng điểm. Vui lòng kiểm tra lại hệ thống.')
        },
      },
    )
  }
          ...
          return (
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                onClick={() => handleExportGrade(student.id!, student.tenHocSinh)}
                className="group cursor-pointer rounded-lg p-2 text-emerald-600 transition-all duration-200 hover:bg-emerald-50 active:bg-emerald-100 disabled:opacity-50"
              >
                {isExportingStudentGrade ? (
                  <Loader2 className="h-4.5 w-4.5 animate-spin" />
                ) : (
                  <FileDown className="h-4.5 w-4.5 transition-transform group-hover:scale-105" />
                )}
              </button>
              ...
              <button
                ...
                className="group cursor-pointer rounded-lg p-2 text-blue-600 transition-all duration-200 hover:bg-blue-50 active:bg-blue-100"
              >
                <Paperclip className="h-4.5 w-4.5 transition-transform group-hover:scale-105 group-hover:rotate-12" />
              </button>
              ...
              {userRole === 'admin' && (
                <button
                  ...
                  className="group cursor-pointer rounded-lg p-2 text-rose-500 transition-all duration-200 hover:bg-rose-50 active:bg-rose-100 disabled:opacity-50"
                >
                  {isRemovingStudent ? (
                    <Loader2 className="h-4.5 w-4.5 animate-spin" />
                  ) : (
                    <Trash2 className="h-4.5 w-4.5 transition-transform group-hover:scale-105" />
                  )}
                </button>
              )}
            </div>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
      <div className="border-b border-gray-100 bg-gray-50/50 px-6 py-4">
        <h2 className="text-lg font-bold text-gray-800">Danh sách học sinh</h2>
      </div>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className="border-b border-gray-100 bg-gray-50/70">
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="px-6 py-3.5 text-xs font-semibold tracking-wider text-gray-500 uppercase"
                  >
