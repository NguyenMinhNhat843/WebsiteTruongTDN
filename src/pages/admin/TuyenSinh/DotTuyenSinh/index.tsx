...
const DotTuyenSinhHome = () => {
  ...
  return (
    <PageShell
      ...
      sub="Quản lý các đợt tuyển sinh và thiết lập chỉ tiêu theo từng năm học."
      icon={Layers}
      renderRight={
        <ButtonAction label="Tạo đợt tuyển sinh" icon={<Plus size={16} />} onClick={handleOpenCreate} />
      }
    >
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-52 animate-pulse rounded-2xl border border-slate-200/60 bg-slate-100"
              />
            ))}
          </div>
