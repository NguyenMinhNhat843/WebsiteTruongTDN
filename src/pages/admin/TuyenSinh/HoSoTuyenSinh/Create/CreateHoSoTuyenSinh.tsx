...
const TaoHoSoTuyenSinh: React.FC = () => {
  ...
  return (
    <PageShell
      title="Tạo Hồ Sơ Tuyển Sinh Mới"
      sub="Thêm mới hồ sơ đăng ký cho học sinh vào hệ thống"
      icon={FileCheck}
      renderRight={
        <div>
          <ButtonAction label="Hủy bỏ" onClick={() => navigate(-1)} variant="outline" className="mr-2" />
          <ButtonAction
            label="Lưu Hồ Sơ"
            type="submit"
        ...
        {selectedCampaignMajorId ? (
              ...
              <div className="grid grid-cols-1 gap-4 border-t border-slate-100 pt-2 sm:grid-cols-3">
                <div>
                  ...
                  <p className="mt-1 text-[10px] text-slate-400">
                    Tính trung bình cộng tất cả điểm môn đã nhập ở mục B, dùng để so với điểm sàn/điểm chuẩn
                    của ngành.
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
            ...
            <p className="mt-1 max-w-sm text-xs text-slate-500">
              Vui lòng hoàn tất bước chọn Ngành, Hệ Đào Tạo và Đợt Tuyển Sinh ở phía trên để mở form nhập
              liệu.
            </p>
          </div>
        )}
