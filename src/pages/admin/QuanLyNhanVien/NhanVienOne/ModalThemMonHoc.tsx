...
import { X, Search, Check, Loader2 } from "lucide-react"; // Import các icon từ lucide-react

interface ModalMonHocGiangDayProps {
  data: SubjectResponseDto[];
  isOpen?: boolean;
  setIsOpen?: (isOpen: boolean) => void;
  onSubmit?: (subjectIds: number[], onSuccess: () => void) => void;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-100 flex flex-col max-h-[85vh] transform transition-all overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {isLoadingSubmit && (
          <div className="absolute inset-0 bg-white/30 z-10 cursor-not-allowed" />
        )}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          ...
          <button
            ...
            className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-6 py-3 border-b border-slate-50 bg-slate-50/50">
          <div className="relative">
            ...
            <input
              ...
              disabled={isLoadingSubmit}
              className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm placeholder-slate-400 text-slate-700 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 transition-all disabled:bg-slate-50 disabled:text-slate-400"
              value={searchTerm}
          ...
          {filteredData.length > 0 ? (
            filteredData.map((subject) => {
              ...
              return (
                <div
                  key={subject.id}
                  className={`flex items-center justify-between p-3.5 rounded-xl border select-none transition-all duration-200 group ${
                    isLoadingSubmit
                ...
                >
                  ...
                  <div
                    ...
                    } ${isLoadingSubmit && isSelected ? "bg-indigo-400 border-indigo-400" : ""}`}
                  >
                    {isSelected && (
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={handleClose}
              disabled={isLoadingSubmit}
              className="px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Hủy bỏ
            </button>
            <button
              ...
              disabled={selectedIds.length === 0 || isLoadingSubmit}
              className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shadow-indigo-100 transition-all flex items-center gap-2"
            >
              {isLoadingSubmit && (
                <Loader2 size={16} className="animate-spin" />
              )}
              {isLoadingSubmit ? "Đang xử lý..." : "Xác nhận"}
            </button>
          </div>
        </div>
      </div>
