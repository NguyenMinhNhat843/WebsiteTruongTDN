...
import { ROLE_COLOR } from "../../../../features/users/constants/user.constants";
import {
  DEPARTMENT,
  STAFF_POSITION,
  type UserResponse,
} from "../../../../features/users/types/User.types";
import { userHelpers } from "../../../../features/users/user.helpers";
import Avatar from "../../../../features/users/components/Avatar";
import InfoRow from "../../../../features/users/components/InfoRow";
import InfoSection from "../../../../features/users/components/InfoSection";
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white rounded-3xl p-7 w-full max-w-160 border border-solid max-h-[90vh] overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="overflow-y-auto custom-scrollbar">
          ...
          {user.staffProfile && (
            <InfoSection title="Thông tin công tác">
              <InfoRow
                label="Mã nhân viên"
                value={user.staffProfile.staffCode}
              />
              <InfoRow
                label="Chức vụ"
                value={
              ...
              {(user.staffProfile.additionalPositions ?? []).length > 0 && (
                <InfoRow
                  ...
                  value={(user.staffProfile.additionalPositions ?? [])
                    .map(
                      (p) => STAFF_POSITION[p as keyof typeof STAFF_POSITION],
                    )
                    .join(", ")}
                />
              )}
        ...
        <div className="flex gap-2 mt-5">
          <ButtonAction label="✏️ Chỉnh sửa" className="border-slate-200" />
          {user.status === "active" ? (
            <ButtonAction
              label="🔒 Khoá TK"
              className="text-red-600 bg-red-50 border border-red-100 rounded-lg hover:bg-red-100"
            />
          ) : (
            <ButtonAction
              label="🔓 Mở khoá"
              className="text-emerald-600 bg-emerald-50 border border-emerald-100 hover:bg-emerald-100"
            />
          )}
          <ButtonAction
            label="🗑 Xoá"
            className="text-slate-400 border-none hover:text-red-600 hover:bg-red-50"
          />
        </div>
      </div>
    </div>
