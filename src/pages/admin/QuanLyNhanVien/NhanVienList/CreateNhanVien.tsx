...
import { useQuanLyNguoiDungContext, type CreateStaffDto } from '../QuanLyNguoiDungContext'
...
const CreateNhanVien = ({ isOpen, onClose }: CreateNhanVienProps) => {
      ...
      await createStaff(
        ...
        },
        {
          ...
          onError: (error: unknown) => {
            toast.error('Lỗi khi tạo nhân viên: ' + ((error as Error).message || JSON.stringify(error)))
          },
        },
      )
    } catch (error) {
  ...
  return (
    <div className="animate-fade-in fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
      ...
      {/* Main Modal */}
      <div className="relative flex max-h-[90vh] w-full max-w-2xl transform flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-2xl transition-all">
        {/* Header với tông xanh giáo dục */}
        <div className="flex items-center justify-between border-b border-blue-100 bg-gradient-to-r from-blue-50 to-indigo-50/50 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800">Thêm Mới Nhân Sự</h3>
              <p className="text-xs text-slate-500">Tạo hồ sơ giáo viên hoặc cán bộ nhân viên mới</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="rounded-lg border border-transparent p-1.5 text-slate-400 shadow-sm transition-all hover:border-slate-100 hover:bg-white hover:text-slate-700"
            type="button"
          >
            <X size={18} />
          </button>
        ...
        <form
          ...
          className="scrollbar-thin flex-1 space-y-6 overflow-y-auto px-6 py-5"
        >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              {/* Giới tính */}
              <SelectOption label="Giới tính" options={genderOptions} {...register('gender')} />
          ...
          {/* Action Buttons */}
          <div className="mt-6 flex items-center justify-end gap-3 border-t border-slate-100 bg-white pt-4">
            <ButtonAction
              type="button"
              variant="outline"
              onClick={onClose}
              ...
              className="border-slate-200 px-5 text-slate-700 hover:bg-slate-50"
            >
              Hủy bỏ
            </ButtonAction>
            ...
            <ButtonAction
              ...
              variant="primary"
              loading={isCreatingStaff}
              className="bg-blue-600 px-5 text-white shadow-lg shadow-blue-100 transition-all duration-150 hover:bg-blue-700 active:bg-blue-800"
            >
              Tạo nhân viên
            </ButtonAction>
          </div>
